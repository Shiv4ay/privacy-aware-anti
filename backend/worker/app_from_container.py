...
5. Cite which document record your answer comes from when possible (e.g., "According to Record 3...").
6. Keep answers concise — 2-4 sentences maximum unless asked for more detail.

## PROFESSIONAL RESPONSE GUIDELINES:
7. ANALYZE all provided context thoroughly — information may be spread across multiple document chunks.
8. SYNTHESIZE information from multiple records when answering aggregate questions (averages, counts, lists).
9. For queries about groups (e.g., "CSE students", "Microsoft placements"):
...
10. The text has been anonymized. When the context contains privacy placeholders like [PERSON:idx_0], [COMPANY:idx_0], [EMAIL:idx_0], [PHONE:idx_0], etc., YOU MUST TREAT THESE AS THE ACTUAL VALUES. DO NOT say the information is missing. You MUST output these literal placeholders EXACTLY as they appear in the context.
...
11. Structure responses with clear formatting:
    - Direct answer to the question using the exact placeholders
    - Supporting details and context
    - For multiple records: use bullet points or tables
    - For aggregates: provide counts, averages, ranges
12. For numerical queries (GPA, counts, averages):
    - Calculate from all relevant records in context
    - Provide exact values and sample sizes
13. Cross-reference related information from multiple document records to give a complete answer.
14. Maintain a professional, analytical tone suitable for academic/enterprise environments.
15. When asked about "all" or "list" queries, compile information from ALL relevant chunks provided.
16. ANSWER ONLY WHAT WAS ASKED. If the user asks for 'name and company', output only Name and Company. Do NOT add unrequested fields like phone, LinkedIn, GPA, email unless explicitly asked.
17. STRICT CITATION FORMAT: When referencing source records, use plain text only, e.g., "According to the student record" or "Based on the alumni record". NEVER use bracket notation like [COMPANY: text] or [RECORD: text] as citation — that format is RESERVED EXCLUSIVELY for anonymized PII tokens."""
        ...
        if lower_path.endswith('.pdf'):
            ...
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                for row_idx, row in enumerate(reader):
                    # Build "key: value" pairs for each field in the row
                    fields = [f"{k}: {v}" for k, v in row.items() if v and str(v).strip()]
                    if fields:
    ...
    try:
        ...
        try:
            ...
            if potential_ids:
                ...
                # Fetch a large limit so all record types (student, alumni, grades) are included before diversification.
...
async def chat_with_documents(req: Request):
    ...
    try:
        ...
        # Also redact any raw PII the LLM sneaked into the response (safety net).
        # This response_pii_map catches any NEW PII in the LLM output.
        response_text, response_pii_map = redact_text(response_text, return_map=True)
        
        # Merge: context map takes priority (it has the accurate placeholder values)
        # Response map adds any additional entries from new PII found in the output
        pii_map = {**response_pii_map, **context_pii_map}  # context_pii_map overwrites response_pii_map
        logger.info(f"FINAL MERGED PII_MAP: {list(pii_map.keys())[:15]}")

        # ── Clean up LLM artifacts: stray `:idx_N]` or `idx_N]` remnants ───────
        # The LLM sometimes echos placeholder fragments alongside real tokens.
        # e.g. "[PERSON:idx_0]:idx_2]" → "[PERSON:idx_0]" (remove the suffix)
        # e.g. "idx_5]" appearing after a badge → removed
        response_text = re.sub(r'(?<=\])\s*:idx_\d+\]', '', response_text)  # ]:idx_N] suffix after a badge
        # !! Use negative lookbehind (?<!:) to ONLY strip standalone `idx_N]` remnants.
        # !! Without this, `[PERSON:idx_0]` → `[PERSON:` (destroying the valid token). 
        response_text = re.sub(r'(?<!:)\bidx_\d+\]', '', response_text)      # standalone idx_N] remnant (NOT inside a token)
        response_text = re.sub(r'(?<=\])\s*\]', '', response_text)           # stray trailing ]
