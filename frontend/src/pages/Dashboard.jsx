...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (!authLoading) {
      ...
      socket.on('activity', (newActivity) => {
        // Update Security Stats (Blocked & Redacted)
        ...
        // Update General Stats (Total Searches)
        setStats(prev => {
          if (!prev) return prev;
          const newStats = { ...prev };
          if (newActivity.action === 'search' || newActivity.action === 'chat') {
            newStats.searches = (newStats.searches || 0) + 1;
          }
          return newStats;
        });
      });

      // Catch backend scheduled stats broadcats
      socket.on('stats_update', (newStats) => {
        if (newStats.totalDocuments) {
          setStats(prev => ({ ...prev, documents: newStats.totalDocuments }));
        }
      });
  ...
  return (
    <>
      <AmbientBackground />
      <div className="space-y-8 animate-fade-in relative z-10 w-full overflow-hidden p-6">
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          >
            {((user?.role !== 'student' && user?.role !== 'guest') || user?.organization_type === 'Personal') && (
              <motion.div variants={staggeredItemVariants}>
                <Link to="/documents/upload" className="block h-full">
                  <AnimatedCard className="glass-panel p-6 rounded-2xl border border-white/5 h-full relative group">
          ...
          >
            ...
            <motion.div variants={staggeredItemVariants} className="lg:col-span-2">
              <AnimatedCard className="glass-panel p-6 rounded-2xl border border-white/10 h-full">
            ...
            {/* Security Info */}
            <motion.div variants={staggeredItemVariants}>
              <AnimatedCard className="glass-panel p-6 rounded-2xl border border-white/10 bg-gradient-to-b from-white/5 to-transparent h-full">
